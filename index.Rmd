...
In this plot it appears that there is a relatively strong relationship between these two variables. The error can be seen to get larger as one approaches the further ends of the best fit line, but regardless of that, there does appear to be some correlated relation between these two variables. 

## Causal Analysis via Average Treatment Effects

```{r}
abort_stats |>
  drop_na(`Contraceptive prevalence, any modern method (% of married women ages 15-49)`) |>
  mutate(contraceptive = ifelse(`Contraceptive prevalence, any modern method (% of married women ages 15-49)` >= 50, "1", "0")) |>
  select(year, request, contraceptive)|>
  group_by(contraceptive) |>
  summarise(ave_request = mean(request, na.rm = TRUE)) |>
  pivot_wider(names_from = contraceptive,
              values_from = ave_request) |>
  mutate(ATE = `1` - `0`) |>
  select(ATE)
```

Through this analysis, I had to establish something that could be viewed as a treatment variable, as the data did not explicitly have experimental varaibles. To do this, I chose that the Contraceptive Prevalence variable would be my treatment, and assumed that a treated group could be those where the prevalence was greater than or equal to 50%. This value is not definitive, and could be subject to change depending on circumstances. Furthermore, I was able to use the original dataset that was not averaged to proportions, as the process of finding ATE would do that for me, thus I was able to keep more data present throughout the process. 

Inn this exploration I found that there does appear to be a relation between having contraceptive prevalence or 50% or higher leading to an increase in reuqest availability of abortions. This value is around 0.28 which suggests that having the higher proportion of contraceptives led to a substantive increase in request availability. 
...
models <- list(
  ...
  "Western Europe" = lm(request ~ `Contraceptive prevalence, any modern method (% of married women ages 15-49)`, data = w_europe),
  "Northern Africa" = lm(request ~ `Contraceptive prevalence, any modern method (% of married women ages 15-49)`, data = n_africa),
  "Eastern Asia" = lm(request ~ `Contraceptive prevalence, any modern method (% of married women ages 15-49)`, data = e_asia))
